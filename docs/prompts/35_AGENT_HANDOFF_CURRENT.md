...
- **P3** (локализация): chart_export + referral_provider (subject/text обязательные)
- `flutter analyze` ✅ 0 issues, `flutter test` ✅ 32/32
...
3. Пользователь копирует ответ в чат OpenCode
4. OpenCode извлекает код и применяет его в проекте
5. `flutter analyze` + `flutter test` + commit

## Текущее состояние проекта
- **Путь:** `C:\Users\a1am3\biorhythms_flutter`
- **Версия:** 1.0.0+1, release APK 26.9MB (установлен на DN2103), release AAB 27.1MB
- **65 Dart-файлов**, **32 теста**, **15+ маршрутов**
- **Последний коммит:** `411c3b1` — fix: add pricing cache + verify fallback for paywall (остальные коммиты не запушены)
- **Build config:** AGP 8.1.0, Kotlin 1.8.22, Gradle 8.3, Java 17 (jvmTarget = "17")
- **Bug Hunt Phase C завершён:** 43 бага #15-#57 найдены, см. `CLAUDE_HANDOFF.md`
...
| **GLM Fix Session (12.07.2026)** | 11 файлов | ✅ Все 3 приоритета из `51_GLM_FIX_PROMPT.md`: DST off-by-one (#5), mounted check (#11), UI overflow (#12), theme colors sweep (5 файлов + AppColors), localization sweep (chart_export + referral_provider). Подробности: `53_GLM_FIX_REPORT.md`. |
...
- **AGP 8.1.0**, **Kotlin 1.8.22**, **Gradle 8.3**, **Java 17** (jvmTarget = "17")
...
flutter test                 ✅ 32/32
flutter build apk --release  ✅ 26.9MB (установлен на DN2103)
flutter build appbundle --release ✅ (27.1MB)
```

## GitHub
- Репо: `https://github.com/1am0nly/byorythms4c`
- `main` — v1.0.0 (запушен: `411c3b1`)
...
## Known Bugs (см. CLAUDE_HANDOFF.md для полного списка #15-#57)
### HIGH (фикс до релиза) — #1-#14 — **ВСЕ ПОЧИНЕНЫ (Phase B, 10.07.2026)**
### Bug Hunt Phase C (#15-#57) — 43 бага найдены 11.07.2026, см. `CLAUDE_HANDOFF.md`
...
## Планирование релиза v1.0.0
- [x] **Выполнен** — Store metadata (Google Play RU, App Store RU)
- [x] **Выполнен** — Release notes v1.0.0
- [x] **Выполнен** — Screenshots specifications (6 скриншотов)
- [x] **Выполнен** — IAP product IDs зафиксированы (`monthly_premium`, `yearly_premium`)
- [x] **Выполнен** — PurchaseProvider рефакторинг (`ref.invalidateSelf()`)
- [x] **Выполнен** — Unit тесты: граничные значения биоритмов
- [x] **Выполнен** — Unit тесты: FemaleCycleData (8 тестов)
- [x] **Выполнен** — #26: ctx.mounted → parent context
- [x] **Выполнен** — #28: TextEditingController dispose
- [x] **Выполнен** — #29, #30: hardcoded emails → AppStrings.developerEmail
- [x] **Выполнен** — use_build_context_synchronously warnings (2 в settings/)

## План фиксов (очередность)
- ✅ **HIGH:** B1-B8 (все баги из code review) — **ИСПРАВЛЕНЫ**
- ✅ **Paywall bugs** (6 багов) — fixed, commits `6f6db41`, `411c3b1`
- ✅ **Bug Hunt Phase C** — 43 бага #15-#57 найдены, см. `CLAUDE_HANDOFF.md`
- ✅ **Build config** — AGP 8.1.0, Kotlin 1.8.22, Gradle 8.3, Java 17
- ✅ **Release APK** — 26.9MB, установлен на DN2103
- **Phase D:** Fix bugs #15-#57 (43 бага)
- **Phase D progress:** #15-28, #26, #28, #29-30 fixed (OpenCode zone done)
- **MEDIUM:** #15-#31
- **LOW:** #32-#42
- **Next:** #45 (🔴 Critical — purchase status), #46-#47 (🟠 High)
...
- [x] **Выполнен** — **Paywall bugs** (6 багов, commits `6f6db41`, `411c3b1`)
- [x] **Выполнен** — **Bug Hunt Phase C** — 43 бага #15-#57 найдены
- [x] **Выполнен** — **Build config** — AGP 8.1.0, Kotlin 1.8.22, Gradle 8.3, Java 17
- [x] **Выполнен** — **Release APK** — 26.9MB, установлен на DN2103
- [ ] **TODO** — Phase D: Fix bugs #15-#57 (OpenCode zone #15-28 + #26, #28, #29-30 done; GLM zone in parallel)
- [ ] **TODO** — #45 (🔴 Critical: purchase status not verified), #46-#47 (🟠 High)
