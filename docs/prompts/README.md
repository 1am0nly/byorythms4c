...
| `CLAUDE_HANDOFF.md` | ✅ Актуальный | Специфичный контекст для **Claude** (code review, architecture, debugging) |
...
3. Прикрепить агент-специфичный файл: `GEMINI_HANDOFF.md` / `CLAUDE_HANDOFF.md` / `DEEPSEEK_HANDOFF.md`
4. **НЕ** давать агентам старые промпты из архива без предупреждения

```
┌─────────────────────────────────────────────────┐
│  OpenCode (локально, код-генерация)             │
│  • виджеты, провайдеры, flutter analyze ✓       │
└───────────────┬───────────────────┬─────────────┘
                │                   │
    код-ревью ← │                   │ → контент / архитектура
                ▼                   ▼
┌───────────────────────┐ ┌───────────────────────┐
│  Claude (онлайн)       │ │  Gemini (онлайн)       │
│  • архитектура        │ │  • code generation    │
│  • глубокая отладка   │ │  • refactoring        │
│  • архитектурные решения │ │  • store content    │
└───────────────────────┘ └───────────────────────┘
         ▲                       ▲
         │                       │
         └───────┬───────────────┘
...
│  • сложные мульти-файловые изменения             │
